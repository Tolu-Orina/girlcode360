...
function requireUser(sub: string) {
  return getUser(sub);
}

function applySyncOp(sub: string, op: SyncOp): void {
  switch (op.op) {
    case "upsert_cycle":
      if (op.cycle.id) upsertCycleWithId(sub, op.cycle.id, op.cycle);
      else createCycle(sub, op.cycle);
      break;
    case "patch_cycle":
      patchCycle(sub, op.id, op.patch);
      break;
    case "delete_cycle":
      deleteCycle(sub, op.id);
      break;
    case "upsert_day":
      upsertDay(sub, op.day);
      break;
  }
}

function cycleSnapshot(sub: string) {
  return {
    cycles: listCycles(sub),
    days: listDays(sub),
    prediction: buildPrediction(sub),
  };
}
...
export const handler = async (
  ...
  if (publicShareMatch && method === "GET") {
    ...
    const result = getPublicShare(publicShareMatch[1]!, apiBase);
  ...
  if (publicShareObj && method === "GET") {
    const meta = getPublicShare(publicShareObj[1]!, "https://api.local");
    if ("error" in meta) return json(404, { error: meta.error });
    const ct = lookupShareCiphertext(publicShareObj[1]!);
  ...
  if (method === "POST" && (stripeHook || paystackHook)) {
    ...
    const result = handleBillingWebhook(provider, body);
  ...
  if (method === "POST" && path === "/v1/privacy/purge-tick") {
    ...
    return json(200, { purged: runDeletionPurge() });
  }
  ...
  if (method === "GET" && path === "/v1/users/me") {
    const profile = getUser(user.sub);
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(200, profile);
  }

  if (method === "PATCH" && path === "/v1/users/me") {
    if (!getUser(user.sub)) return json(404, { error: "user_not_bootstrapped" });
    return json(200, upsertUser(user.sub, parseBody<PatchUserRequest>(event)));
  }
  ...
  if (method === "PATCH" && path === "/v1/users/me/modules") {
    ...
    return json(200, setModules(user.sub, body.modules as HealthModule[]));
  }
  ...
  if (method === "POST" && path === "/v1/consents") {
    ...
    const created = addConsents(
      user.sub,
      body.jurisdiction as Market,
      policyVersion,
      body.items,
    );
    return json(201, { created, current: latestConsentsByPurpose(user.sub) });
  }

  if (method === "GET" && path === "/v1/consents") {
    if (!getUser(user.sub)) return json(404, { error: "user_not_bootstrapped" });
    return json(200, {
      current: latestConsentsByPurpose(user.sub),
      history: listConsents(user.sub),
    });
  }
  ...
  if (!requireUser(user.sub) && path.startsWith("/v1/cycles")) {
    return json(404, { error: "user_not_bootstrapped" });
  }

  if (method === "GET" && path === "/v1/cycles") {
    if (!requireUser(user.sub)) return json(404, { error: "user_not_bootstrapped" });
    return json(200, cycleSnapshot(user.sub));
  }
  ...
  if (method === "POST" && path === "/v1/cycles") {
    ...
    const cycle = createCycle(user.sub, body);
    return json(201, { cycle, prediction: buildPrediction(user.sub) });
  }

  if (method === "GET" && path === "/v1/cycles/predictions") {
    if (!requireUser(user.sub)) return json(404, { error: "user_not_bootstrapped" });
    return json(200, buildPrediction(user.sub));
  }
  ...
  if (method === "GET" && path === "/v1/cycles/days") {
    ...
    return json(200, { days: listDays(user.sub, from, to) });
  }
  ...
  if (method === "PUT" && path === "/v1/cycles/days") {
    ...
    return json(200, { day: upsertDay(user.sub, body) });
  }
  ...
  if (method === "POST" && path === "/v1/cycles/sync") {
    ...
    const cached = getIdempotent(user.sub, key);
    if (cached) return json(200, cached);

    const body = parseBody<SyncRequest>(event);
    const ops = body.ops ?? [];
    for (const op of ops) applySyncOp(user.sub, op);
    const response = {
      idempotencyKey: key,
      applied: ops.length,
      ...cycleSnapshot(user.sub),
    };
    setIdempotent(user.sub, key, response);
  ...
  if (cycleMatch) {
    ...
    if (id === "predictions" || id === "days" || id === "sync") {
      ...
      const cycle = getCycle(user.sub, id);
      if (!cycle) return json(404, { error: "cycle_not_found" });
      return json(200, { cycle });
    } else if (method === "PATCH") {
      const cycle = patchCycle(user.sub, id, parseBody<PatchCycleRequest>(event));
      if (!cycle) return json(404, { error: "cycle_not_found" });
      return json(200, { cycle, prediction: buildPrediction(user.sub) });
    } else if (method === "DELETE") {
      if (!deleteCycle(user.sub, id)) return json(404, { error: "cycle_not_found" });
      return json(200, { ok: true, prediction: buildPrediction(user.sub) });
    }
  }
  ...
  if (method === "GET" && path === "/v1/pcos/biometrics") {
    const from = event.queryStringParameters?.from;
    const to = event.queryStringParameters?.to;
    return json(200, { biometrics: listBiometrics(user.sub, from, to) });
  }

  if (method === "PUT" && path === "/v1/pcos/biometrics") {
    const body = parseBody<UpsertBiometricRequest>(event);
    if (!body.date) return json(400, { error: "date_required" });
    return json(200, { biometric: upsertBiometric(user.sub, body) });
  }

  if (method === "GET" && path === "/v1/pcos/medications") {
    return json(200, { medications: listMedications(user.sub) });
  }
  ...
  if (method === "POST" && path === "/v1/pcos/medications") {
    ...
    return json(201, { medication: createMedication(user.sub, body) });
  }
  ...
  if (medMatch) {
    ...
    if (method === "DELETE") {
      if (!deleteMedication(user.sub, id)) {
  ...
  if (method === "GET" && path === "/v1/pcos/insights") {
    return json(200, {
      insights: pcosInsightsForUser(user.sub),
      disclaimer:
  ...
  if (method === "POST" && path === "/v1/pcos/push-subscription") {
    ...
    const saved = savePushSubscription(user.sub, body);
  ...
  if (method === "GET" && path === "/v1/pcos/reminders/due") {
    ...
    const due = dueMedications(user.sub, nowLocal);
    return json(200, {
      due,
      pushPayload: {
        title: "GirlCode360",
      ...
      subscriptions: listPushSubscriptions(user.sub).length,
    });
  }
  ...
  if (method === "GET" && path === "/v1/emergency") {
    ...
    return json(200, {
      ...
      numbers: emergencyNumbers(profile.market),
    });
  }

  if (method === "GET" && path === "/v1/notifications/prefs") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(200, { prefs: getNotificationPrefs(user.sub) });
  }

  if (method === "PATCH" && path === "/v1/notifications/prefs") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(200, {
      prefs: patchNotificationPrefs(
        user.sub,
  ...
  if (method === "GET" && path === "/v1/pregnancy") {
    const status = pregnancyStatus(user.sub);
  ...
  if (method === "POST" && path === "/v1/pregnancy") {
    ...
    const profilePreg = initPregnancy(user.sub, body);
    const week = pregnancyStatus(user.sub)?.week ?? 1;
  ...
  if (method === "GET" && path === "/v1/pregnancy/days") {
    return json(200, { days: listPregnancyDays(user.sub) });
  }

  if (method === "PUT" && path === "/v1/pregnancy/days") {
    const body = parseBody<UpsertPregnancyDayRequest>(event);
    if (!body.date) return json(400, { error: "date_required" });
    return json(200, { day: upsertPregnancyDay(user.sub, body) });
  }

  if (method === "GET" && path === "/v1/pregnancy/appointments") {
    return json(200, { appointments: listAppointments(user.sub) });
  }
  ...
  if (method === "POST" && path === "/v1/pregnancy/appointments") {
    ...
    return json(201, { appointment: createAppointment(user.sub, body) });
  }

  const apptMatch = path.match(/^\/v1\/pregnancy\/appointments\/([^/]+)$/);
  if (apptMatch && method === "DELETE") {
    if (!deleteAppointment(user.sub, apptMatch[1]!)) {
  ...
  if (method === "GET" && path === "/v1/ttc") {
    const status = ttcStatus(user.sub);
  ...
  if (method === "POST" && path === "/v1/ttc") {
    ...
    return json(201, initTtc(user.sub, body.startedOn));
  }

  if (method === "GET" && path === "/v1/ttc/fertile-window") {
    return json(200, fertileForUser(user.sub));
  }

  if (method === "GET" && path === "/v1/ttc/days") {
    return json(200, { days: listTtcDays(user.sub) });
  }

  if (method === "PUT" && path === "/v1/ttc/days") {
    const body = parseBody<UpsertTtcDayRequest>(event);
    if (!body.date) return json(400, { error: "date_required" });
    const result = upsertTtcDay(user.sub, body);
  ...
  if (intimacyMatch && method === "DELETE") {
    if (!deleteTtcIntimacy(user.sub, intimacyMatch[1]!)) {
  ...
  if (method === "GET" && path === "/v1/wallet/docs") {
    runWalletPurge();
    return json(200, { docs: listWalletDocs(user.sub) });
  }
  ...
  if (method === "POST" && path === "/v1/wallet/uploads") {
    ...
    try {
      const host = event.headers?.Host ?? event.headers?.host ?? "api.local";
      const apiBase = process.env.API_BASE_URL ?? `https://${host}`;
      const result = createWalletUpload(user.sub, body, apiBase);
      return json(201, {
        doc: result.doc,
  ...
  if (objMatch) {
    ...
    if (method === "PUT") {
      ...
      if (!putWalletObject(user.sub, id, b64)) {
        return json(404, { error: "doc_not_found" });
      }
      return json(200, { ok: true });
    }
    if (method === "GET") {
      const ct = getWalletObject(user.sub, id);
      if (!ct) return json(404, { error: "object_missing" });
      const doc = getWalletDoc(user.sub, id);
      return json(200, {
        ciphertextB64: ct,
        fileIv: doc?.fileIv,
  ...
  if (docMatch) {
    ...
    if (method === "PATCH") {
      ...
      const doc = patchWalletMeta(user.sub, id, body);
      if (!doc) return json(404, { error: "doc_not_found" });
      return json(200, { doc });
    }
    if (method === "DELETE") {
      const doc = softDeleteWalletDoc(user.sub, id);
      if (!doc) return json(404, { error: "doc_not_found" });
      return json(200, {
        doc,
        purgePolicy:
  ...
  if (shareCreateMatch && method === "POST") {
    const result = createWalletShare(
      user.sub,
      shareCreateMatch[1]!,
  ...
  if (shareCreateMatch && method === "GET") {
    return json(200, {
      shares: listSharesForDoc(user.sub, shareCreateMatch[1]!),
    });
  }

  const revokeMatch = path.match(/^\/v1\/wallet\/shares\/([^/]+)$/);
  if (revokeMatch && method === "DELETE") {
    if (!revokeWalletShare(user.sub, revokeMatch[1]!)) {
  ...
  if (method === "GET" && path === "/v1/zara/quota") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(200, { quota: getZaraQuota(user.sub) });
  }
  ...
  if (method === "POST" && path === "/v1/zara/chat") {
    ...
    if (mode === "context") {
      const consents = latestConsentsByPurpose(user.sub);
  ...
  if (method === "GET" && path === "/v1/zara/context-preview") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(200, { context: assembleZaraContext(user.sub) });
  }

  if (method === "GET" && path === "/v1/healthlens/status") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(200, getHealthLensStatus(user.sub));
  }
  ...
  if (method === "POST" && path === "/v1/healthlens/population-consent") {
    ...
    return json(200, {
      status: setPopulationLearningConsent(user.sub, Boolean(body.granted)),
    });
  }
  ...
  if (method === "GET" && path === "/v1/healthlens/report") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    const report = latestHealthLensReport(user.sub);
  ...
  if (method === "POST" && path === "/v1/healthlens/prep-card") {
    ...
    const card = buildPrepCard(user.sub, body.questions ?? []);
  ...
  if (method === "POST" && path === "/v1/privacy/export") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(201, { job: createExportJob(user.sub) });
  }

  const exportMatch = path.match(/^\/v1\/privacy\/export\/([^/]+)$/);
  if (exportMatch && method === "GET") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    const job = getExportJob(user.sub, exportMatch[1]!);
  ...
  if (method === "POST" && path === "/v1/privacy/delete") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(201, { deletion: requestDeletion(user.sub) });
  }

  if (method === "POST" && path === "/v1/privacy/delete/cancel") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    const deletion = cancelDeletion(user.sub);
  ...
  if (method === "GET" && path === "/v1/privacy/delete") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(200, { deletion: getDeletion(user.sub) });
  }

  if (method === "GET" && path === "/v1/billing/status") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(200, getBillingStatus(user.sub));
  }
  ...
  if (method === "POST" && path === "/v1/billing/checkout") {
    ...
    return json(200, createCheckoutSession(user.sub, provider, body.successUrl));
  }

  if (method === "POST" && path === "/v1/billing/portal") {
    if (!profile) return json(404, { error: "user_not_bootstrapped" });
    return json(200, createPortalSession(user.sub));
  }
  ...
  if (method === "POST" && path === "/v1/billing/dev-activate") {
    ...
    return json(200, { status: activatePremium(user.sub, "dev") });
  }
